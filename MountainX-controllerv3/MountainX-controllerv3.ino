...
const unsigned char logo[] PROGMEM = {
  ...
  0x00, 0x0E, 0x00, 0x00, 0x1F, 0x00, 0x00, 0x1F, 0x01, 0x80, 0x1F, 0x03,
  0x80, 0xBF, 0x03, 0xC0, 0xFF, 0x06, 0xC0, 0xE5, 0x06, 0x60, 0x60, 0x04,
  0x20, 0x00, 0x0C, 0x30, 0x00, 0x0C, 0x30, 0x00, 0x08, 0x18, 0x00, 0x18,
  0x08, 0x00, 0x18, 0x0C, 0x00, 0x10, 0x0C, 0x00, 0x30, 0x06, 0x00, 0x20,
  0xBE, 0xBB, 0x7D, 0xFE, 0xFF, 0x7F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
};
...
const short rules[numOfSettings][3] {
  ...
  {72, 0, 250}, // Wheel pulley
  {165, 0, 250}, // Wheel diameter
...
const uint8_t hallSensorPin = A1;
//const uint8_t CE = 9;
//const uint8_t CS = 10;
...
const uint8_t hallNoiseMargin = 50;
const uint8_t hallMenuMargin = 150;
...
void setup() {
  ...
  DEBUG_PRINT(INPUT_PULLUP);
  DEBUG_PRINT(triggerPin);
  DEBUG_PRINT(hallSensorPin);
  DEBUG_PRINT(batteryMeasurePin);
...
void startAdv(void)
{
  DEBUG_PRINT("Bluefruit52 BLEUART Example");
  DEBUG_PRINT("---------------------------\n");
  // Advertising packet
...
void transmitToReceiver(){
  ...
  if ( millis() - lastTransmission >= 50 ) {
    ...
    if ( BLEWriteP()) //bleuart.print( &remPackage))) //, sizeof(remPackage) ) )
    {
      // Listen for an acknowledgement reponse (return of uart data).
      uint8_t buf[sizeof(returnData)];
      while (bleuart.available()) { // radio.isAckPayloadAvailable()) {
        bleuart.read( buf, sizeof(returnData) );
      }
...
bool transmitSetting(uint8_t setting, uint64_t value){
  ...
  /* Handeled differently on BLEUart
  // Lets clear the ack-buffer (so it can be used to confirm the new setting).
  while ( bleuart.isAckPayloadAvailable() && settingWaitDelay >= ( millis() - beginTime) ) {
    bleuart.read( &returnData, sizeof(returnData) );
    delay(100);
  }
  */
  ...
  while ( !payloadSend && settingWaitDelay >= (millis() - beginTime) ){
    ...
    /* TEMPORERRY< NEEDS TO BE LOOCKED AT!!!!!!!!! TODO:
  ...
  if(payloadSend == true){
    ...
    while ( !ackRecieved && settingWaitDelay >= ( millis() - beginTime) ){
      ...
      if( bleuart.write( &setPackage, sizeof(setPackage) ) ){

        delay(100);

        while( radio.isAckPayloadAvailable() && !ackRecieved ){
          DEBUG_PRINT( F("TX <-- Acknowledgement") );
          bleuart.read( &setPackage, sizeof(setPackage) );
          ackRecieved = true;
        }
      }
    }
...
size_t BLEWriteP() {
  size_t size = sizeof(remPackage);
  size = bleuart.write( &remPackage.type, size);
  size = bleuart.write( reinterpret_cast<uint8_t*>((remPackage).throttle >> 8), size);
  size = bleuart.write( reinterpret_cast<uint8_t*>((remPackage).throttle & 0xff), size);
  return bleuart.write( &remPackage.trigger, size);
}
